...
class TestStrictMultiFileComparison:
    ...
    def test_discover_all_file_pairs(self):
        ...
        assert len(self.file_pairs) >= 4, f"Expected at least 4 file pairs, found {len(self.file_pairs)}"
    ...
    def test_strict_gps_coordinates_all_files(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            # Find best alignment
            best_offset = self._find_gps_alignment(fit_lat, fit_lon, csv_lat, csv_lon)
            print(f"   Best alignment offset: {best_offset}")
            
            # Test with very strict tolerance
            exact_matches = 0
            test_count = min(100, len(fit_lat) - abs(best_offset), len(csv_lat) - abs(best_offset))
            ...
            print(f"   Exact GPS matches: {exact_matches}/{test_count} ({match_ratio:.1%})")
            
            assert match_ratio > 0.95, f"{fit_file.name}: GPS should match >95%, got {match_ratio:.1%}"
    ...
    def test_strict_distance_all_files(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            best_offset = self._find_distance_alignment(fit_distance_km, csv_distance_km)
            print(f"   Distance alignment offset: {best_offset}")
            
            # Test exact matches with proper alignment
            exact_matches = 0
            close_matches = 0
            test_count = min(100, len(fit_distance_km) - abs(best_offset), len(csv_distance_km) - abs(best_offset))
            
            for i in range(test_count):
                fit_idx = i + max(0, best_offset)
                csv_idx = i + max(0, -best_offset)
                
                if fit_idx >= len(fit_distance_km) or csv_idx >= len(csv_distance_km):
                    break
                ...
                elif csv_val > 0 and abs(fit_val - csv_val) / csv_val < 0.0000001:
                    close_matches += 1
            
            exact_ratio = exact_matches / test_count if test_count > 0 else 0
            close_ratio = (exact_matches + close_matches) / test_count if test_count > 0 else 0
            
            print(f"   Exact distance matches: {exact_matches}/{test_count} ({exact_ratio:.1%})")
            print(f"   Close distance matches: {exact_matches + close_matches}/{test_count} ({close_ratio:.1%})")
            
            assert close_ratio > 0.98, f"{fit_file.name}: Distance should match >98%, got {close_ratio:.1%}"
    ...
    def test_heart_rate_all_files(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            exact_matches = 0
            test_count = min(50, len(fit_hr) - abs(best_offset), len(csv_hr) - abs(best_offset))
            ...
            valid_tests = sum(1 for i in range(test_count) 
                            if (i + max(0, best_offset) < len(fit_hr) and 
                                i + max(0, -best_offset) < len(csv_hr) and
                                50 <= fit_hr[i + max(0, best_offset)] <= 220 and
                                50 <= csv_hr[i + max(0, -best_offset)] <= 220))
            
            if valid_tests > 0:
                match_ratio = exact_matches / valid_tests
                print(f"   Heart rate exact matches: {exact_matches}/{valid_tests} ({match_ratio:.1%})")
                assert match_ratio > 0.8, f"{fit_file.name}: Heart rate should match >80%, got {match_ratio:.1%}"

    def _find_gps_alignment(self, fit_lat: List[float], fit_lon: List[float], 
                           csv_lat: List[float], csv_lon: List[float]) -> int:
        ...
        for offset in range(-5, 6):
            score = 0
            test_count = min(50, len(fit_lat) - abs(offset), len(csv_lat) - abs(offset))
    ...
    def _find_distance_alignment(self, fit_dist: List[float], csv_dist: List[float]) -> int:
        ...
        for offset in range(-5, 6):
            score = 0
            test_count = min(50, len(fit_dist) - abs(offset), len(csv_dist) - abs(offset))
            ...
            for i in range(test_count):
                ...
                if csv_dist[csv_idx] > 0:
                    diff_percent = abs(fit_dist[fit_idx] - csv_dist[csv_idx]) / csv_dist[csv_idx]
    ...
    def _find_heart_rate_alignment(self, fit_hr: List[int], csv_hr: List[int]) -> int:
        ...
        for offset in range(-3, 4):
            score = 0
            test_count = min(30, len(fit_hr) - abs(offset), len(csv_hr) - abs(offset))
            ...
            for i in range(test_count):
                ...
                if (50 <= fit_hr[fit_idx] <= 220 and 50 <= csv_hr[csv_idx] <= 220 and
                    fit_hr[fit_idx] == csv_hr[csv_idx]):
                    score += 1
