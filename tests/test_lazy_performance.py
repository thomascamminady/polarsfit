...
class TestLazyPerformance:
    ...
    def test_lazy_creation_vs_eager_simulation(self):
        ...
        start_time = time.time()
        eager_attempts = 0
        for i in range(min(5, num_operations)):  # Limit to avoid too many errors
            try:
        ...
        if eager_attempts > 0:
            estimated_eager_total = (eager_simulation_time / eager_attempts) * num_operations
        else:
            estimated_eager_total = 1.0  # Fallback estimate
        
        print(f"\nPerformance Comparison:")
        print(f"Lazy {num_operations} LazyFrames: {lazy_time:.4f}s")
        print(f"Estimated eager {num_operations} DataFrames: {estimated_eager_total:.4f}s")
        print(f"Speedup: {estimated_eager_total/lazy_time:.1f}x")
        
        # Assertions
        assert len(lazy_frames) == num_operations
        assert lazy_time < 0.1, f"Lazy creation took {lazy_time:.4f}s, should be < 0.1s"
        assert lazy_time < estimated_eager_total / 5, "Lazy should be at least 5x faster"
    ...
    def test_lazy_operation_chaining_performance(self):
        ...
        operations = [
            ...
            lambda x: x.with_columns((pl.col('power') * 1.1).alias('adjusted_power')),
        ...
        print(f"Chained {len(operations) * 10} operations in {chaining_time:.4f}s")
        
        assert isinstance(result_lf, pl.LazyFrame)
        assert chaining_time < 0.1, f"Operation chaining took {chaining_time:.4f}s, should be < 0.1s"
    ...
    def test_memory_efficiency_lazy_vs_eager(self):
        ...
        assert memory_increase < 50, f"Memory increase of {memory_increase:.2f}MB too high"
    ...
    def test_error_handling_performance(self):
        ...
        print(f"Created {len(lazy_frames)} LazyFrames with invalid paths in {creation_time:.4f}s")
        
        assert len(lazy_frames) == len(invalid_paths) * 20
        assert creation_time < 0.1, "Even with invalid paths, creation should be fast"
    ...
    def test_scalability_many_lazy_frames(self):
        ...
        for batch_size in batch_sizes:
            ...
            print(f"Created {batch_size} LazyFrames in {batch_time:.4f}s "
        ...
        assert time_per_frame_1000 < time_per_frame_100 * 2, "Scaling should be roughly linear"
    ...
    def test_concurrent_lazy_operations(self):
        ...
        print(f"Average thread time: {sum(thread_times)/len(thread_times):.4f}s")
