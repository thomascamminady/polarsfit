...
class TestAdaptiveMultiFileComparison:
    ...
    def test_all_file_pairs_basic_integrity(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            assert df_fit.shape[0] > 0, (
                f"FIT file {fit_file.name} has no records"
            )
            assert df_csv.shape[0] > 0, (
                f"CSV file {csv_file.name} has no records"
            )

            # Core FIT fields should exist
            assert "field_253" in df_fit.columns, (
                f"Missing timestamp in {fit_file.name}"
            )

            print(
                f"   ✅ FIT: {df_fit.shape[0]} records, {df_fit.shape[1]} fields"
            )
            print(
                f"   ✅ CSV: {df_csv.shape[0]} records, {df_csv.shape[1]} fields"
            )
    ...
    def test_gps_coordinates_when_available(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            if (
                "Latitude" not in df_csv.columns
                or "Longitude" not in df_csv.columns
            ):
            ...
            print(
                f"   GPS close matches: {close_matches}/{test_count} ({match_ratio:.1%})"
            )

            # GPS should be reasonably close (allowing for export precision differences)
            assert match_ratio > 0.8, (
                f"{fit_file.name}: GPS should match >80%, got {match_ratio:.1%}"
            )
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            for i in range(test_count):
                ...
                if csv_val > 0:
                    ...
                    abs(fit_val - csv_val) < 0.000001
                ):  # Absolute tolerance for zero values
                    close_matches += 1

            match_ratio = close_matches / test_count if test_count > 0 else 0
            print(
                f"   Distance close matches: {close_matches}/{test_count} ({match_ratio:.1%})"
            )

            assert match_ratio > 0.95, (
                f"{fit_file.name}: Distance should match >95%, got {match_ratio:.1%}"
            )
    ...
    def test_heart_rate_when_available(self):
        ...
        for fit_file, csv_file in self.file_pairs:
            ...
            if (
                "field_3" not in df_fit.columns
                or "HeartRate" not in df_csv.columns
            ):
            ...
            for i in range(test_count):
                ...
                if (
                    fit_val is None
                    or csv_val is None
                    or not isinstance(fit_val, (int, float))
                    or not isinstance(csv_val, (int, float))
                    or fit_val <= 30
                    or fit_val >= 250
                    or csv_val <= 30
                    or csv_val >= 250
            ...
            if valid_comparisons > 10:
                ...
                print(
                    f"   Heart rate exact matches: {exact_matches}/{valid_comparisons} ({exact_ratio:.1%})"
                )
                print(
                    f"   Heart rate close matches: {exact_matches + close_matches}/{valid_comparisons} ({close_ratio:.1%})"
                )

                # Allow for timing differences and sensor variations
                assert close_ratio > 0.7, (
                    f"{fit_file.name}: Heart rate should match >70%, got {close_ratio:.1%}"
                )
            else:
                print(
                    f"   ⏭️  Insufficient valid heart rate data: {valid_comparisons} comparisons"
                )
