...
class TestLazyEvaluation:
    ...
    def test_lazy_creation_instant(self):
        ...
        assert creation_time < 0.01, f"LazyFrame creation took {creation_time:.4f}s, should be < 0.01s"
    ...
    def test_lazy_operations_instant(self):
        ...
        assert operations_time < 0.01, f"Operation chaining took {operations_time:.4f}s, should be < 0.01s"
    ...
    def test_error_deferred_until_collection(self):
        ...
        with pytest.raises(Exception):  # Should be a file not found error
            filtered.collect()
    ...
    def test_lazy_vs_eager_performance_simulation(self):
        ...
        assert lazy_creation_time < 0.1, f"Creating 100 LazyFrames took {lazy_creation_time:.4f}s, should be < 0.1s"
        
        print(f"Created 100 LazyFrames in {lazy_creation_time:.4f}s ({lazy_creation_time/100*1000:.2f}ms per LazyFrame)")
    ...
    def test_lazy_query_optimization_potential(self):
        ...
        optimized_query = (lf
                          .filter(pl.col('heart_rate') > 150)  # Early filter
        ...
        assert 'SELECT' in query_plan or 'SELECTION' in query_plan  # Should contain selection
    ...
    def test_lazy_scan_data_functionality(self):
        ...
        assert isinstance(lf, pl.LazyFrame)
        assert creation_time < 0.01, f"scan_data LazyFrame creation took {creation_time:.4f}s"
    ...
    def test_multiple_lazy_operations_without_execution(self):
        ...
        start_time = time.time()
        result_lf = lf
        for i in range(50):  # Many operations
            result_lf = result_lf.filter(pl.col('heart_rate') > i)
        
        chaining_time = time.time() - start_time
        
        assert isinstance(result_lf, pl.LazyFrame)
        assert chaining_time < 0.1, f"50 operation chains took {chaining_time:.4f}s"
    ...
    def test_lazy_frame_memory_efficiency(self):
        ...
        final_objects = len([obj for obj in globals().values() if obj is not None])
        
        # The number of objects should grow, but each LazyFrame should be lightweight
        assert len(lazy_frames) == 1000
        print(f"Created 1000 LazyFrames. Object count change: {final_objects - initial_objects}")
    ...
    def test_lazy_correctness_with_field_mapping(self):
        ...
        start_time = time.time()
        lf = polarsfit.scan_recordmesgs('test_file.fit', field_mapping=field_mapping)
        creation_time = time.time() - start_time
        ...
        with pytest.raises(Exception):  # Will fail on non-existent file
            lf.collect()

    def test_lazy_explain_shows_query_plan(self):
        """Test that lazy frames can show their execution plan."""
        lf = polarsfit.scan_recordmesgs('test_file.fit')
        filtered = lf.filter(pl.col('heart_rate') > 150).select(['timestamp', 'heart_rate'])
    ...
    def test_performance_benchmark_lazy_vs_simulated_eager(self):
        ...
        print(f"Lazy is {estimated_eager_time/lazy_total_time:.1f}x faster for creation")
        
        # Lazy should be significantly faster
        assert lazy_total_time < estimated_eager_time / 10, "Lazy should be at least 10x faster than eager"
