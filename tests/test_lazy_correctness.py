...
class TestLazyCorrectness:
    ...
    def test_lazy_vs_eager_identical_interface(self):
        ...
        assert lazy_params == eager_params, f"Parameter mismatch: lazy={lazy_params}, eager={eager_params}"
    ...
    def test_lazy_field_mapping_preservation(self):
        ...
        lf_data = polarsfit.scan_data('test.fit', 'record', field_mapping=field_mapping)
        assert isinstance(lf_data, pl.LazyFrame)
    ...
    def test_lazy_optimization_hints(self):
        ...
        filtered_first = (lf
                         .filter(pl.col('heart_rate') > 150)  # Should push down
                         .select(['timestamp', 'heart_rate']))  # Should prune columns
    ...
    def test_lazy_consistency_with_eager_interface(self):
        ...
        lazy_mapped = polarsfit.scan_recordmesgs('test.fit', field_mapping=mapping)
        assert isinstance(lazy_mapped, pl.LazyFrame)
        
        # Test 3: scan_data variants
        lazy_data1 = polarsfit.scan_data('test.fit', 'record')
        lazy_data2 = polarsfit.scan_data('test.fit', 'session', field_mapping=mapping)
